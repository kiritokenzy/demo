...
def add_to_startup(program_name, path_to_exe):
    ...
    #     try:
    #         if reg.QueryValueEx(key, program_name)[0] == path_to_exe:
    #             return
    #         else:
    #             reg.SetValueEx(key, program_name, 0, reg.REG_SZ, path_to_exe)
...
def create_shortcut_to_startup():
    # Lấy đường dẫn đến thư mục Startup
    startup_path = os.path.join(os.environ["APPDATA"], r"Microsoft\Windows\Start Menu\Programs\Startup")

    # Lấy đường dẫn đầy đủ của file .exe hiện tại
    exe_path = os.path.abspath(sys.argv[0])

    # Tạo đường dẫn cho shortcut
    shortcut_path = os.path.join(startup_path, os.path.basename(exe_path))

    # Sao chép file .exe vào thư mục Startup
